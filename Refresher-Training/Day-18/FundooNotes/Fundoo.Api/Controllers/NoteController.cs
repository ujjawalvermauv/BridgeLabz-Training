...
public class NoteController : ControllerBase
{
    ...
    public async Task<IActionResult> GetRecent()
    {
        ...
        var result = await _noteService.GetRecentNotesAsync(userId);
    ...
    {
        var result = await _noteService.AddLabelAsync(noteId, labelId, GetUserId());
        return result ? Ok("Label attached successfully.") : BadRequest("Unable to attach label. Ensure note and label exist and belong to you.");
    }

    [HttpDelete("{noteId}/label/{labelId}")]
    public async Task<IActionResult> RemoveLabel(int noteId, int labelId)
    {
        var result = await _noteService.RemoveLabelAsync(noteId, labelId, GetUserId());
        return result ? Ok("Label removed successfully.") : BadRequest("Unable to remove label.");
    }
